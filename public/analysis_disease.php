...
if(isset($month) && isset($year) && $month != '' && $year != '') {
  ...
  $sql_count = 'SELECT d1.p_curr_diag as p_curr_diag,(SELECT count(p.p_sex) FROM patient p INNER JOIN diagnosis d ON p.p_pid=d.p_pid WHERE d1.p_curr_diag=d.p_curr_diag AND p.p_sex=1 AND MONTH(p.created)=' . $month . ' AND YEAR(p.created)=' . $year . ') as p_dis_male,(SELECT count(p.p_sex) FROM patient p INNER JOIN diagnosis d ON p.p_pid=d.p_pid WHERE d1.p_curr_diag=d.p_curr_diag AND p.p_sex=2 AND MONTH(p.created)=' . $month . ' AND YEAR(p.created)=' . $year . ') as p_dis_female,(SELECT count(p.p_sex) FROM patient p INNER JOIN diagnosis d ON p.p_pid=d.p_pid WHERE d1.p_curr_diag=d.p_curr_diag AND p.p_sex=3 AND MONTH(p.created)=' . $month . ' AND YEAR(p.created)=' . $year . ') as p_dis_other FROM diagnosis d1 GROUP BY(d1.p_curr_diag)
';
            ...
            <ul class="dropdown-menu multi-level" role="menu" aria-labelledby="dropdownMenu">
              ...
              <li class="dropdown-submenu active">
                ...
                <ul class="dropdown-menu">
                  ...
                  <li class="active"><a href="./analysis_disease.php">Disease vs Gender</a></li>
                </ul>
              </li>
            </ul>
          </li>
      ...
      <?php
        if($result->num_rows > 0)  {
          ...
          while($row = $result->fetch_assoc())  {
            echo "<tr>" .
                    "<td>" . $row['p_curr_diag'] . "</td>" .
                    "<td>" . $row['p_dis_male'] . "</td>" .
                    "<td>" . $row['p_dis_female'] . "</td>" .
                    "<td>" . $row['p_dis_other'] . "</td>" .
                 "</tr>";
      ?>
